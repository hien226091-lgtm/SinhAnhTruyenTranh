...
from api_base.app.models.schema_db import Comic, Character, Frame, User 
...
async def upload_nhan_vat(
    ...
    comic_id: int = Form(None), # Nhận ComicID từ Frontend
    files: list[UploadFile] = File(...),
    db: Session = Depends(get_db) # Mở kết nối Database
) -> CharacterUploadResponse:
    """Upload character reference images and store them in DB."""
    if not files:
    ...
    for index, upload_file in enumerate(files, start=1):
        ...
        target_path = session_inputs_dir / f"nhan_vat_{index}_{int(time.time())}{suffix}"
        ...
        try:
            ...
            # --- LƯU VÀO DATABASE BẢNG CHARACTERS ---
            if comic_id:
                new_char = Character(
                    ComicID=comic_id,
                    ReferenceImages=file_url # Tạm lưu URL ảnh vào DB
                )
                db.add(new_char)
            # ----------------------------------------
...
def san_xuat_truyen(
    ...
    """Produce individual images and save frames to DB."""
    ...
    manifest_path = CONFIG.outputs_dir / f"anh_manifest_{payload.comic_id or 'default'}.json"
